            ...
            <el-tab-pane label="待退款流水">
                ...
                <div class="accounting-actions with-margin-top">
                    <el-button type="primary" @click="downloadToPay">
                        <i class="iconfont icon-download"></i>下载 待退款清单
                    </el-button>
                </div>
            </el-tab-pane>
            <el-tab-pane label="已退款记录">
                ...
                <div class="accounting-actions with-margin-top">
                    <el-upload
                        ...
                        <el-button type="warning"><i class="iconfont icon-upload"></i>导入 退款结果</el-button>
                    </el-upload>
                </div>
            </el-tab-pane>
...
<script>
  ...
  import {refundHeaders} from "~/common/constants";
  ...
  export default {
    data() {
      return {
        ...
        toPay: [],
        uploadData: {
          file_type: 'buyer_refund',
        }
      }
    },
    methods: {
      downloadToPay() {
        exportCSVFile(refundHeaders, this.resources, '待退款清单' + timeToDateString(new Date()))
      },
      ...
      loadData() {
        this.$axios.get('user/refunds').then(response => {
          this.toPay = response.data.data;
        });
      }
    },
    mounted() {
      this.initResources('user/finance', null, {file_type: 'buyer_refund'});
      this.loadData();
    }
  }
</script>
